...
void SpriteAnimation::Render(const glm::mat4& vp, const glm::mat4& model)
{
	// 1. Animation Logic (Calculating which frame to show)
	if (animation)
	{
		...
		if (animation->isPlaying)
		{
			...
			if (animation->frame > animation->end + 1) {
				animation->frame = static_cast<float>(animation->start);
			}
		}

		// Get UV coordinates from Atlas
		TextureAtlasEntry* m_pCurrentEntry = m_pTextureAtlas->GetEntryByName(oss.str());
		if (m_pCurrentEntry) {
			uvOffset = m_pCurrentEntry->GetTextureOffset();
			uvScale = m_pCurrentEntry->GetTextureScale();
		}
	}
	else {
		// Fallback for static sprites
		uvOffset = glm::vec2(0.0f, 0.0f);
		uvScale = glm::vec2(1.0f, 1.0f);
	}

	// 2. Call the cleaned Renderer
	// We pass 'vp' and 'model' separately as per our new QuadRenderer::DrawQuad signature
...
void SpriteAnimation::Save(tinyxml2::XMLDocument* document)
{
	...
	xmlFileNodeAnimations.Write("width", size.x);
	xmlFileNodeAnimations.Write("height", size.y);
